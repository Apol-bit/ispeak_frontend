...
class ScriptDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String fullContent =
        script['transcript'] ??
        script['content'] ??
        'No content available for this script.';
...
class _ScriptPracticePageState extends State<ScriptPracticePage> {
    ...
    try {
      if (await _audioRecorder.hasPermission()) {
        ...
        if (_status == PracticeStatus.ready) {
          ...
          _audioPath =
              '${tempDir.path}/ispeak_script_${DateTime.now().millisecondsSinceEpoch}.wav';
  ...
  Future<void> _finishSession() async {
    ...
    try {
      ...
      if (finalPath != null) {
        ...
        final scriptLanguage = widget.script['language'];
        request.fields['language'] =
            ['English', 'Filipino', 'Taglish'].contains(scriptLanguage)
            ? scriptLanguage
            : 'English';
        final resourceId = widget.script['_id'];
  ...
  Widget _buildTeleprompter(String title, String level, String language) {
    final isActive =
        _status == PracticeStatus.recording || _status == PracticeStatus.paused;
    final String content =
        widget.script['transcript']?.toString() ??
        widget.script['content']?.toString() ??
        '';
              ...
              Container(
                ...
                child: SingleChildScrollView(
                  ...
                  padding: const EdgeInsets.symmetric(
                    horizontal: 20,
                    vertical: 110,
                  ), // Padding so text can start at the reading line
                  child: Text(
                    content,

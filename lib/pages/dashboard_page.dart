...
class _DashBoardPageState extends State<DashBoardPage> {
  ...
  Future<void> _fetchDashboardData() async {
    try {
      ...
      if (statsRes.statusCode == 200) {
        ...
        debugPrint(
          'Dashboard Stats: totalSessions=$totalSessions, avgScore=$overallScore, dayStreak=$currentStreak',
        );
      }
  ...
  Widget _headerWithOverlappingCard(BuildContext context) {
    ...
    final double headerHeight = (r.h(178) + topPadding).clamp(
      160.0 + topPadding,
      210.0 + topPadding,
    );
    return Stack(
        ...
        children: [
          Row(
            ...
            children: [
              Expanded(
                child: Column(
                  // NEW: Wrap date in column to add time below
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(
                      date,
          ...
          Row(
            children: [
              Expanded(
                child: _Metric(label: 'Pace', value: pace),
              ),
              Expanded(
                child: _Metric(label: 'Clarity', value: clarity),
              ),
              Expanded(
                child: _Metric(label: 'Energy', value: energy),
              ),
            ],
          ),
        ],
